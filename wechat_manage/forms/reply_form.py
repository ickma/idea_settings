...
class BaseReplyForm(BaseForm):
    ...
    text = forms.CharField(widget=forms.Textarea, label=u'添加回复语', required=False)
    # todo 实现素材的自动选择
...
class KeyWorldReplyForm(BaseForm):
    # 设置关键字回复
    name = forms.CharField(label=u'名称')
    keywords = forms.CharField(widget=forms.Textarea(attrs={'placeholer': u'多个关键字请单独另起一行'}), label=u'关键字')
