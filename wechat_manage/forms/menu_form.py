...
class MenuCreateForm(BaseForm):
    ...
    menu_level = forms.IntegerField(
        widget=forms.Select(choices=[(1, u'1级菜单'), (2, u'2级菜单')]), label=u'菜单级别')
    ...
    def __init__(self, data=None, files=None, auto_id='id_%s', prefix=None, initial=None, error_class=ErrorList,
        ...
        if initial['menu_level'] == 1:
            self.fields['parent_menu'].initial = initial['parent_menu']
        ...
        elif initial.get('menu_cates') == 'as_parent':
            self.fields['menu_cates'].initial = 'as_parent'

        if action == 'edit':
            """在编辑模式下，不允许更改菜单级别"""
            self.fields['menu_level'].widget.attrs['disabled'] = 'disabled'
            self.fields['menu_level'].initial = initial['menu_level']
